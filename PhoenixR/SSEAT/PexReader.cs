...
namespace PhoenixEngineR.SSEAT
{
    ...
    public class StringParam
    {
        ...
        public string GetTextIfTransR()
        {
            ...
            if (PhoenixR.TransData.ContainsKey(GetKey))
            {
                GetResult = PhoenixR.TransData[GetKey];
            }
    ...
    public class PexReader
    {
        ...
        public void LoadPexFile(string FilePath)
        {
            ...
            foreach (var GetFile in DataHelper.GetAllFile(Bridge.GetFullPath(@"Cache\")))
            {
                if (File.Exists(GetFile.FilePath) && GetFile.FilePath.Contains(Bridge.GetFullPath(@"Cache\")))
                {
        ...
        public bool DeCodeFileToUsing(string FilePath)
        {
            ...
            if (File.Exists(Bridge.GetFullPath(@"Cache\") + PathInFo.FileName + "." + PathInFo.FileType))
            {
                ForceDelete(Bridge.GetFullPath(@"Cache\") + PathInFo.FileName + "." + PathInFo.FileType);
            }

            File.Copy(FilePath, Bridge.GetFullPath(@"Cache\") + PathInFo.FileName + "." + PathInFo.FileType);
            TempFilePath = Bridge.GetFullPath(@"Cache\") + PathInFo.FileName + "." + PathInFo.FileType;

            string TempPath = Bridge.GetFullPath(@"Cache\") + PathInFo.FileName + "." + PathInFo.FileType;
            CallChampollion(Bridge.GetFullPath(@"Tool\Champollion.exe"), TempPath, Bridge.GetFullPath(@"Cache\"), Bridge.GetFullPath(@"Cache\"));

            //Wait for Champollion decompilation to complete
            WaitChampollion();

            string CurrentPath = Bridge.GetFullPath(@"Cache\");
        ...
        {
            string CompilerPath = "";
            if (Bridge.FindPapyrusCompilerPath(ref CompilerPath))
            {
                ...
                if (File.Exists(Bridge.GetFullPath(@"\") + TempPathInFo.FileName + "." + TempPathInFo.FileType))
                {
                    ForceDelete(Bridge.GetFullPath(@"\") + TempPathInFo.FileName + "." + TempPathInFo.FileType);
                }

                File.Copy(TempFilePath, Bridge.GetFullPath(@"\") + TempPathInFo.FileName + "." + TempPathInFo.FileType);
                ...
                string SetDisassemblePath = Bridge.GetFullPath(TempPathInFo.FileName + "." + "disassemble.pas");

                if (File.Exists(SetDisassemblePath))
                {
                    string SetCachePas = Bridge.GetFullPath(@"Cache\") + TempPathInFo.FileName + "." + "pas";
                    ...
                    string SetSource = Bridge.GetFullPath(TempPathInFo.FileName + "." + TempPathInFo.FileType);
                    ...
                    if (File.Exists(TempFilePath))
                    {
                        if (TempFilePath.Contains(Bridge.GetFullPath(@"\Cache\")))
                        {
        ...
        public bool SavePexFile(string OutPutPath)
        {
            ...
            {
                if (PhoenixR.TransData.ContainsKey(GetParam.Key))
                {
                    LinkTexts.Add(new LinkValue(GetParam.SourceText, PhoenixR.TransData[GetParam.Key], GetParam.LineID, GetParam.Key));
                }
            }

            var Encoding = DataHelper.GetFileEncodeType(Bridge.GetFullPath(@"\Cache\" + CurrentFileName + ".pas"));

            string GetFileContent = Encoding.GetString(DataHelper.ReadFile(Bridge.GetFullPath(@"\Cache\" + CurrentFileName + ".pas")));
            ...
            for (int i = 0; i < LinkTexts.Count; i++)
            {
                ...
                if (GetFileContent.Contains(GetOriginal))
                {
                    ...
                    NormalizePunctuation(ref GetTranslated);
            ...
            string SetPath = Bridge.GetFullPath(@"\Cache\" + CurrentFileName + ".pas");
            ...
            if (File.Exists(Bridge.GetFullPath(@"\") + CurrentFileName + ".pas"))
            {
                ForceDelete(Bridge.GetFullPath(@"\") + CurrentFileName + ".pas");
            }

            DataHelper.WriteFile(Bridge.GetFullPath(@"\") + CurrentFileName + ".pas", Encoding.GetBytes(GetFileContent));
            ...
            if (Bridge.FindPapyrusCompilerPath(ref CompilerPath))
            {
                ...
                File.Copy(Bridge.GetFullPath(@"\" + CurrentFileName + ".pas"), GetWorkPath + CurrentFileName + ".pas");
